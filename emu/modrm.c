...
void parse_modrm(Emulator* emu, ModRM* modrm)
{
    ...
    memset(modrm, 0, sizeof(ModRM)); // 0に初期化
    ...
    // opecode: 0x38 = b00111000
    modrm->opecode = ((code & 0x38) >> 3);
    // rm:      0x07 = b00000111
    modrm->rm = code & 0x07;

    // 8bit(1byte)読み取ったのでeip(次に読み込むアドレス番地)を1byte分進める
    emu->eip += 1;

    // modが3(10) = b11以外かつrmが4(10) = b100の場合にSIBが存在する
    if (modrm->mod != 3 && modrm->rm == 4) {
        modrm->sib = get_code8(emu, 0);
        emu->eip += 1;
    }

    // modが0(10) = b0かつrmが5(10) = b101を満たすか、modが2(10) = b10のとき32bitのディスプレースメントが存在する
    // modが1(10) = b1の場合に8bitのディスプレースメントが存在する
